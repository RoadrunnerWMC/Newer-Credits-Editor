...
# Print some stuff
print('[[ Freezing Newer Credits Editor ]]')
...
setup(
    name = 'Newer Credits Editor',
    version = Version,
    description = 'Newer Credits Editor',
    options={
        'build_exe': {
            'excludes': excludes,
...
if upxFlag:
    if os.path.isfile('upx/upx.exe'):
        ...
        print('>> In order to build Newer Credits Editor with UPX, place the upx.exe file into '
...
print('>> Newer Credits Editor has been frozen to %s !' % dir_)
...
# Compile using py2exe
# Copied, pasted, and edited from Reggie
from distutils.core import setup
from py2exe.build_exe import py2exe
import os, os.path, shutil, sys

if '-upx' in sys.argv: sys.argv.remove('-upx')

dir = 'bin'
prog = 'Newer Credits Editor'
ver = '1.0'

print '[[ Freezing %s ]]' % prog
...
setup(
    ...
    description=prog,
    windows=[
        {'script': 'Newer_Credits_Editor.py'}
    ],
    options={'py2exe':{
...
# now that it's built, configure everything
try: os.unlink(dir + '/w9xpopen.exe') # not needed
except: pass
...
print '>> %s has been frozen to %s!' % (prog, dir)
